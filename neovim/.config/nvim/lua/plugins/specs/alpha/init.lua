...
function M.config()
  local headers = require("plugins.specs.alpha.headers")
  local quotes = require("plugins.specs.alpha.quotes")
  ...
  local function apply_gradient_hl(text)
    local gradient = require("plugins.specs.alpha.utils").create_gradient("#DCA561", "#658594", #text)
  ...
  local function get_footer(quotes, width)
    ...
    for _, text in ipairs(quote_text) do
      local padded_text = require("plugins.specs.alpha.utils").pad_string(text, max_width, "right")
  ...
  theme.config.layout = {
    ...
    get_mru(10),
    { type = "padding", val = 3 },
    get_footer({ quotes.beauty, quotes.rilke }, 50),
  }
